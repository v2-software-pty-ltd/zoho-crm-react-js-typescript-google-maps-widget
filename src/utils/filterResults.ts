import {
  ...
  MinMaxNumberType,
  SalesTypeArray,
  NameOnTitlePropertyType,
  NameOnTitleContactType,
} from "../types";
...
function matchForPropertyTypes(
  property: UnprocessedResultsFromCRM,
  desiredPropertyTypes: string[]
): boolean {
  return desiredPropertyTypes.some((propertyType: string) => {
    return (
      desiredPropertyTypes.includes("All") ||
      property.Property_Category_Mailing.includes(propertyType)
    );
  });
}

function matchForPropertyGroups(
  property: UnprocessedResultsFromCRM,
  desiredPropertyGroups: string[]
): boolean {
  return desiredPropertyGroups.some((propertyGroup: string) => {
    return (
      desiredPropertyGroups.includes("All") ||
      property.Property_Type_Portals.includes(propertyGroup)
    );
  });
}
...
function getOwnerData(property: UnprocessedResultsFromCRM) {
  ...
  const parsedPropertyContacts = !property.Property_Contacts
    ? []
    : JSON.parse(property.Property_Contacts);
  parsedPropertyContacts.forEach((contact: OwnerType) => {
    contact.Contact_Type = "Director";
    ownerData.push(contact);
  });

  const parsedPropertyOwners = !property.Property_Owners
    ? []
    : JSON.parse(property.Property_Owners);
...
function getNameOnTitlePropertyData(property: UnprocessedResultsFromCRM) {
  ...
  const parsedNameOnTitleProperty = !property.Name_on_Title_data_cache
    ? []
    : JSON.parse(property.Name_on_Title_data_cache);
  parsedNameOnTitleProperty.forEach((owner: NameOnTitlePropertyType) => {
...
function getNameonTitleContactData(property: UnprocessedResultsFromCRM) {
  ...
  const parsedNameOnTitleProperty = !property.Name_on_Title_contacts_cache
    ? []
    : JSON.parse(property.Name_on_Title_contacts_cache);
  parsedNameOnTitleProperty.forEach((contact: NameOnTitleContactType) => {
...
function checkSalesOrLeaseFilter(
  searchParams: IntersectedSearchAndFilterParams
) {
  const defaultBaseFilterKeys = Object.keys(DEFAULT_BASE_FILTER_PARAMS);
  const newSearchParamObject: IntersectedSearchAndFilterParams = Object.assign(
    {},
    searchParams
  );
  ...
  const salesAndLeaseSearchParamValues: (undefined | boolean | number)[] =
    Object.values(newSearchParamObject).flatMap(
      (valueObject: SaleTypeEnum[] | MinMaxDateType | MinMaxNumberType) => {
        if (Array.isArray(valueObject)) {
          return SalesTypeArray.some((salesType: SaleTypeEnum) =>
            valueObject.includes(salesType)
          );
        } else {
  ...
  return salesAndLeaseSearchParamValues.some(
...
export default function filterResults(
  unsortedPropertyResults: UnprocessedResultsFromCRM[][],
  ...
  searchParameters.forEach(
    (searchParams: IntersectedSearchAndFilterParams, index: number) => {
      ...
      const maxResultsForPropertyTypes: number =
        searchParams.propertyTypesMaxResults;
      const maxResultsForPropertyGroups: number =
        searchParams.propertyGroupsMaxResults;
      ...
      const isPropertyFiltersInUse =
        isPropertyGroupFilterInUse || isPropertyTypeFilterInUse;
      const isManagedFilterInUse = desiredManaged !== "All";
      const isBaseFiltersInUse =
        isPropertyGroupFilterInUse ||
        isPropertyTypeFilterInUse ||
        isManagedFilterInUse;
      const isPropertyTypeOrGroupMaxRecordInUse =
        searchParams.propertyTypesMaxResults !== Infinity ||
        searchParams.propertyGroupsMaxResults !== Infinity;
      const isNeighbourMaxInUse =
        searchParams.neighboursSearchMaxRecords !== Infinity;
      const isSalesOrLeaseFiltersInUse = checkSalesOrLeaseFilter(searchParams);
      const isSubFilterInUse =
      ...
      const areAnyFiltersBesidesNeighbourFilterEnabled =
        (isBaseFiltersInUse || isSalesOrLeaseFiltersInUse) &&
        !isNeighbourMaxInUse;
      ...
      unsortedPropertyResults[index].forEach(
        (property: UnprocessedResultsFromCRM) => {
          const isUnderNeighbourLimit =
            matchTallies.neighbour < maxNumNeighbours;
          const isUnderPropertyTypeLimit =
            areAnyFiltersBesidesNeighbourFilterEnabled &&
            matchTallies.propertyType < maxResultsForPropertyTypes;
          const isUnderPropertyGroupLimit =
            areAnyFiltersBesidesNeighbourFilterEnabled &&
            matchTallies.propertyGroup < maxResultsForPropertyGroups;
          const canAddBasedOnMaxResults =
            isUnderNeighbourLimit ||
            isUnderPropertyTypeLimit ||
            isUnderPropertyGroupLimit;

          if (canAddBasedOnMaxResults) {
            const propertyTypeMatch =
              isPropertyTypeFilterInUse &&
              isUnderPropertyTypeLimit &&
              matchForPropertyTypes(property, desiredPropertyTypes);
            const propertyGroupMatch =
              isPropertyGroupFilterInUse &&
              isUnderPropertyGroupLimit &&
              matchForPropertyGroups(property, desiredPropertyGroups);
            const propertyGroupAndTypeMatch =
              propertyGroupMatch || propertyTypeMatch;
            ...
            if (isSubFilterInUse) {
              ...
              if (filterInUse === "SalesEvidenceFilter") {
                ...
                canAddBasedOnFilters =
                  canAddBasedOnFilters && salesOrLeaseMatch;
              }
              if (filterInUse === "LeasesEvidenceFilter") {
                salesOrLeaseMatch = leasesEvidenceFilter(
                  searchParams,
                  property
                );
                canAddBasedOnFilters =
            ...
            const isManaged =
              typeof property.Managed === "string"
                ? property.Managed === desiredManaged ||
                  desiredManaged === "All"
                : property.Managed;
            ...
            if (
              isManagedFilterInUse &&
              !isPropertyFiltersInUse &&
              !isSalesOrLeaseFiltersInUse
            ) {
              // N.B. used to show all properties that are managed
              shouldAddProperty = isManaged;
            } else if (
              isManagedFilterInUse &&
              (isPropertyFiltersInUse || isSalesOrLeaseFiltersInUse)
            ) {
              if (isSubFilterInUse) {
                ...
                shouldAddProperty = isPropertyFiltersInUse
                  ? (isManaged && salesOrLeaseMatch) || canAddBasedOnFilters
                  : isManaged && salesOrLeaseMatch;
              } else {
            ...
            if (isSearchMultiProperties) {
              shouldAddMultiSearchProperty = !searchMultiPropertyDupes.includes(
                property.id
              );
            }
            ...
            if (shouldAddProperty && shouldAddMultiSearchProperty) {
              ...
              if (filterInUse !== "LeasesEvidenceFilter") {
                ...
                const nameOnTitlePropertyData =
                  getNameOnTitlePropertyData(property);
                property.NameOnTitlePropertyDetails = nameOnTitlePropertyData;
                //Use to remove duplicate based on Name on title if data is greater than 1
                if (nameOnTitlePropertyData.length > 1) {
                  for (let i = 0; i < nameOnTitlePropertyData.length; ++i)
                    for (let j = 0; j < nameOnTitlePropertyData.length; ++j)
                      if (
                        i !== j &&
                        nameOnTitlePropertyData[i].Name_On_Title.id ===
                          nameOnTitlePropertyData[j].Name_On_Title.id
                      )
                        nameOnTitlePropertyData.splice(j, 1);
                  property.NameOnTitlePropertyDetails = nameOnTitlePropertyData;
                }
                const nameOnTitleContactData =
                  getNameonTitleContactData(property);
                property.NameOnTitleContactDetails = nameOnTitleContactData;
                //Use to remove duplicate based on Name on title if data is greater than 1
                if (nameOnTitleContactData.length > 1) {
                  for (let i = 0; i < nameOnTitleContactData.length; ++i)
                    for (let j = 0; j < nameOnTitleContactData.length; ++j)
                      if (
                        i !== j &&
                        nameOnTitleContactData[i].Contact.id ===
                          nameOnTitleContactData[j].Contact.id &&
                        nameOnTitleContactData[i].Name_On_Title.id ===
                          nameOnTitleContactData[j].Name_On_Title.id
                      )
              ...
              let canAddToNeighbourCountBasedOnFilters: boolean | string =
                canAddBasedOnFilters;
              if (filterInUse === "BaseFilter") {
                // Managed in base filter
                if (isManagedFilterInUse && !isPropertyFiltersInUse) {
                  canAddToNeighbourCountBasedOnFilters =
                    !canAddBasedOnFilters && !isManaged;
                ...
                if (isManagedFilterInUse && !isPropertyFiltersInUse) {
                  canAddToNeighbourCountBasedOnFilters =
                    !salesOrLeaseMatch || !isManaged;
                } else if (isManagedFilterInUse && isPropertyFiltersInUse) {
                  if (isPropertyTypeOrGroupMaxRecordInUse) {
                    canAddToNeighbourCountBasedOnFilters =
                      filterInUse === "LeasesEvidenceFilter"
                        ? (!isManaged && !propertyGroupAndTypeMatch) ||
                          !salesOrLeaseMatch
                        : !isManaged && !propertyGroupAndTypeMatch;
                  } else {
                    canAddToNeighbourCountBasedOnFilters =
